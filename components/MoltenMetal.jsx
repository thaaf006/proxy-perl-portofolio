...
const MoltenMetal = ({
  ...
  useEffect(() => {
    ...
    const tryStart = () => {
      if (motionEnabled && isVisible && isPageVisible && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
      if (raf !== 0) {
  ...
  }, [maxDpr, motionEnabled, mouseInteraction]);
  ...
  useEffect(() => {
    ...
    if (!motionEnabled) ctx.renderer.render({ scene: ctx.mesh });
  }, [
    color1,
    color2,
    color3,
