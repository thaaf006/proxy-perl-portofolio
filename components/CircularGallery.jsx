...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Documentation photographs are wide prints, rather than portrait cards.
    this.plane.scale.y = (this.viewport.height * (640 * this.scale)) / this.screen.height;
...
class App {
  ...
  onTouchMove(e) {
    ...
    if (e.cancelable && Math.abs(x - this.start) > Math.abs(y - this.startY)) e.preventDefault();
    const distance = (this.start - x) * (this.scrollSpeed * 0.025);
    this.scroll.target = this.scroll.position + distance;
    if (this.reducedMotion) this.update();
  }
  onTouchUp() {
    if (this.isDown && this.dragDistance < 7 && this.medias?.[0] && this.onItemClick) {
      const width = this.medias[0].width;
      const rawIndex = Math.round(this.scroll.current / width);
      const index = ((rawIndex % this.itemCount) + this.itemCount) % this.itemCount;
  ...
  update() {
    ...
    if (!this.reducedMotion) this.raf = window.requestAnimationFrame(this.update.bind(this));
  }
  setActive(active) {
...
export default function CircularGallery({
  ...
  scrollSpeed = 2,
  scrollEase = 0.05,
  onItemClick = /** @type {((index: number) => void) | undefined} */ (undefined),
  reducedMotion = false
}) {
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      const observer = new IntersectionObserver(([entry]) => app?.setActive(entry.isIntersecting), { rootMargin: '120px' });
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onItemClick, reducedMotion]);
  return (
    <div
